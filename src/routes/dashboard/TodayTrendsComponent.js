...
for (let x = 1; x <= 24; x++) {
    data.push({label:`jan${x}`, x: x, y: Math.floor(Math.random() * 100) });
}
...
function TodayTrendsComponent({product, cath,cus}) {
    ...
    return (
        ...
        >
            ...
            <Column flexGrow={3} flexBasis='342px' breakpoints={{ 1024: classes.stats }}>
                {renderStat('Total Income', '449')}
                {renderStat('Total Profit', '426')}
                {renderStat('Total Products', product?.length)}
                {renderStat('Total Customers', cus?.length)}
                {renderStat('Total Employess', '60')}
            </Column>
        </Row>
    );
}
